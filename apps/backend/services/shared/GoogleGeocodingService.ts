...
// Re-export VerifiedVenue from GooglePlacesService for backwards compatibility
export type { VerifiedVenue } from "./GooglePlacesService";
...
export class GoogleGeocodingService {
  ...
  /** Called after construction to wire up the circular dependency with GooglePlacesService. */
  setPlacesService(ps: GooglePlacesService): void {
...
${userCityState ? `User is in ${userCityState}.` : userCoordinates ? `User coordinates: ${userCoordinates.lat.toFixed(5)},${userCoordinates.lng.toFixed(5)}` : ""}`,
      ...
      if (address) {
        ...
        if (!this.placesService) {
          throw new Error("PlacesService not wired — call setPlacesService() after construction");
        }
