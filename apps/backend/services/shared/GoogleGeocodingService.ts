      ...
      if (result) {
        ...
        if (result.types && result.types.length > 0) {
          const relevantTypes = result.types
            .filter(
              (type: string) => !["point_of_interest", "establishment", "university"].includes(type)
            )
        ...
        if (
          locationContext &&
          !details.some((detail) => locationContext.toLowerCase().includes(detail.toLowerCase()))
        ) {
...
${
  ...
  > {
    try {
      ...
      // Use the Places API for better search results
      const url = "https://places.googleapis.com/v1/places:searchText";
  ...
  public async getLocationFromCoordinates(
    ...
    try {
      ...
      const country =
        addressComponents.find((component: any) => component.types.includes("country"))
          ?.long_name || "";
