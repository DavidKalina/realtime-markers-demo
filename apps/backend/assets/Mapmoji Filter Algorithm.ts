...
class MapMojiEventFilter {
  ...
  private preFilter(events: Event[]): Event[] {
    ...
    return events.filter(event => {
      ...
      if (!event.isRecurring && event.eventDate < now) {
        const hoursSincePast = (now.getTime() - event.eventDate.getTime()) / (1000 * 60 * 60);
        if (hoursSincePast > 24) { // Hide events more than 24h past
          return false;
        }
      }
      
      // Must not be too far in the future
      const hoursInFuture = (event.eventDate.getTime() - now.getTime()) / (1000 * 60 * 60);
      if (hoursInFuture > 24 * 30) { // Hide events more than 30 days future
        return false;
      }
  ...
  private calculateTimeScore(event: Event): number {
    ...
    const relevantTime = event.isRecurring 
      ? this.getNextRecurrence(event, now) 
      : eventTime;
    
    if (!relevantTime) return 0;
    
    const hoursUntilEvent = (relevantTime.getTime() - now.getTime()) / (1000 * 60 * 60);
    ...
    if (hoursUntilEvent < 0) {
      ...
      const decay = (this.config.timeDecayHours - hoursUntilEvent) / this.config.timeDecayHours;
  ...
  private calculateDistanceScore(event: Event): number {
    if (!this.config.userLocation) {
      ...
      const centerLat = (this.config.viewportBounds.north + this.config.viewportBounds.south) / 2;
      const centerLng = (this.config.viewportBounds.east + this.config.viewportBounds.west) / 2;
  ...
  private getDistanceScore(lat1: number, lng1: number, lat2: number, lng2: number): number {
    ...
    if (distance <= this.config.maxDistanceKm) {
      ...
      return 0.2 + (0.4 * (this.config.maxDistanceKm - distance) / (this.config.maxDistanceKm - 15));
    }
    return 0.1; // Too far
  }
  ...
  private calculateRecencyScore(event: Event): number {
    const now = this.config.currentTime;
    const hoursOld = (now.getTime() - event.createdAt.getTime()) / (1000 * 60 * 60);
  ...
  private applyRelativeScoring(scoredEvents: EventScore[]): EventScore[] {
    ...
    const sortedByRawScore = [...scoredEvents].sort((a, b) => b.rawScore - a.rawScore);
    ...
    return scoredEvents.map((eventScore, index) => {
      const rankPosition = sortedByRawScore.findIndex(se => se.event.id === eventScore.event.id);
      const percentileRank = scoredEvents.length === 1 ? 1.0 : (scoredEvents.length - rankPosition - 1) / (scoredEvents.length - 1);
      ...
      if (scoreRange > 0) {
        const normalizedRawScore = (eventScore.rawScore - minRawScore) / scoreRange;
        ...
        relativeScore = (relativeScore * (1 - qualityWeight)) + (normalizedRawScore * qualityWeight);
      }
  ...
  private getNextRecurrence(event: Event, from: Date): Date | null {
    ...
    const startDate = event.recurrenceStartDate || event.eventDate;
...
export class FilterProcessor {
  private filter: MapMojiEventFilter;

  constructor() {
    // Default configuration - make these configurable per request
    ...
    this.filter = new MapMojiEventFilter(config);
  }
  ...
  async processEventRequest(
    ...
    this.filter = new MapMojiEventFilter({
      ...this.filter['config'],
    ...
    return await this.filter.filterEvents(events);
  }
}