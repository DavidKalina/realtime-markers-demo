...
export const ip = () => {
    return async (c: Context, next: Next) => {
        ...
        const proxyIPs = forwardedFor ? forwardedFor.split(",").map(ip => ip.trim()) : [];
        const clientIP = proxyIPs[0] || realIP || cfConnectingIP || trueClientIP || c.req.header("x-client-ip") || c.req.raw.headers.get("x-client-ip") || "unknown";
