...
export const rateLimit = (options: RateLimitOptions) => {
    ...
    const defaultKeyGenerator = (c: Context) => {
        ...
        return c.get("user")?.userId || c.req.header("x-forwarded-for") || c.req.header("x-real-ip") || "unknown";
    };
    ...
    return async (c: Context, next: Next) => {
        ...
        try {
            const { limited, remaining, resetTime } = await rateLimitService.isRateLimited(key, {
