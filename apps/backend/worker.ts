  ...
  async function processJobs() {
    ...
    try {
      ...
      if (job.type === "cleanup_outdated_events") {
        ...
        // Check if this is a dummy event request
        if (job.data.isDummy) {
          await jobQueue.updateJobStatus(jobId, {
            progress: "Generating dummy event...",
          });

          // Generate random coordinates near the user's location if provided
          const baseLat = job.data.userCoordinates?.lat || 37.7749;
          const baseLng = job.data.userCoordinates?.lng || -122.4194;
          const randomLat = baseLat + (Math.random() - 0.5) * 0.1;
          const randomLng = baseLng + (Math.random() - 0.5) * 0.1;

          // Generate random event date within next 30 days
          ...
          // List of possible emojis and categories
          const emojis = ["🎉", "🎵", "🎸", "🎭", "🎨", "🎤", "🎬", "🎮", "🏆", "🍔"];
          ...
          const dummyEvent = {
            ...
            title: `Dummy Event ${Math.floor(Math.random() * 1000)}`,
            eventDate: eventDate,
            location: {
              type: "Point" as const,
            ...
            address: `${Math.floor(Math.random() * 1000)} Test Street, Test City`,
            locationNotes: "Test location notes",
            categoryIds: [categories[Math.floor(Math.random() * categories.length)].id],
            creatorId: job.data.creatorId,
            qrDetectedInImage: false,
            originalImageUrl: "https://example.com/dummy-image.jpg",
          };

          // Create the event
          const newEvent = await eventService.createEvent(dummyEvent);
