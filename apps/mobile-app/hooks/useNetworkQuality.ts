...
export const useNetworkQuality = () => {
  const [networkState, setNetworkState] =
    useState<NetworkQualityState>(DEFAULT_STATE);
  ...
  const handleNetworkStateChange = useCallback(
    (state: NetInfoState) => {
      ...
      try {
        ...
        const isWifiEnabled =
          networkType === "wifi" && (state.isConnected ?? false);
        const isCellularEnabled =
          networkType === "cellular" && (state.isConnected ?? false);

        // If we're not connected, reset the network type
        const effectiveType =
          (state.isConnected ?? false) ? networkType : "none";
          ...
          details: {
            cellularGeneration:
              ...
              "cellularGeneration" in state.details
                ...
                : null,
            wifiStrength:
              isWifiEnabled && state.details && "strength" in state.details
          ...
          (nextAppState: AppStateStatus) => {
            ...
            if (nextAppState === "active") {
              ...
              NetInfo.fetch()
                ...
                .catch((error) => {
                  console.error(
                    "Error fetching network state on app active:",
                    error,
                  );
                });
            }
