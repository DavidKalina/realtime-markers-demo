...
export const useMapWebSocket = (
  ...
  const setStoreMarkers = config.useLocationStore.getState().setMarkers; // Direct store access
  ...
  const selectMarkerFromStoreRef = useRef(
    config.useLocationStore.getState().selectMarker,
  );
  const selectedMarkerIdFromStoreRef = useRef(
    config.useLocationStore.getState().selectedMarkerId,
  );
  ...
  const emitMarkersUpdated = useCallback(
    (
      ...
      if (selectedId) {
        ...
        if (
          ...
          config.eventBroker.emit(config.EventTypes.MARKER_DESELECTED, {
            timestamp: Date.now(),
      ...
      config.eventBroker.emit(config.EventTypes.MARKERS_UPDATED, {
        timestamp: Date.now(),
        source: "useMapWebSocket",
    ...
    [config], // Dependencies are managed via refs now
  );
  ...
  const updateViewport = useCallback(
    (viewport: MapboxViewport) => {
      ...
      config.eventBroker.emit(config.EventTypes.VIEWPORT_CHANGED, {
        timestamp: Date.now(),
        source: "useMapWebSocket",
        viewport: viewport,
    ...
    [sendViewportUpdateToServer, config], // Dependency
  );
  ...
  const handleWebSocketMessage = useCallback(
      ...
      try {
        ...
        switch (data.type) {
          ...
          case MessageTypes.REPLACE_ALL: // Server sends this for full viewport/filter updates
            ...
            console.log(
              "[useMapWebsocket] REPLACE_ALL - Events array:",
              data.events,
            );
            console.log(
              "[useMapWebsocket] REPLACE_ALL - Civic engagements array:",
              data.civicEngagements,
            );
            if (!Array.isArray(data.markers)) {
              console.warn(
                "[useMapWebsocket] Invalid markers array in REPLACE_ALL",
                data,
              );
          ...
          case MessageTypes.ADD_EVENT: {
            ...
            try {
              ...
              if (added) {
                config.eventBroker.emit(config.EventTypes.MARKER_ADDED, {
                  timestamp: Date.now(),
                  source: "useMapWebSocket",
                  markers: [newMarker],
                  count: 1,
          ...
          case MessageTypes.DELETE_EVENT: {
            ...
            try {
              ...
              if (deleted) {
                ...
                config.eventBroker.emit(config.EventTypes.MARKER_REMOVED, {
                  timestamp: Date.now(),
                  source: "useMapWebSocket",
                  markers: [
          ...
          case MessageTypes.ADD_CIVIC_ENGAGEMENT: {
            if (!data.marker || !data.marker.id) {
              console.warn(
                "[useMapWebsocket] Missing or invalid marker data in ADD_CIVIC_ENGAGEMENT",
              );
              return;
            }
            try {
              ...
              console.error(
                "[useMapWebsocket] Error processing ADD_CIVIC_ENGAGEMENT:",
                e,
              );
            }
            break;
          ...
          case MessageTypes.UPDATE_CIVIC_ENGAGEMENT: {
            if (!data.marker || !data.marker.id) {
              console.warn(
                "[useMapWebsocket] Missing or invalid marker data in UPDATE_CIVIC_ENGAGEMENT",
              );
              return;
            }
            try {
              ...
              setMarkers((prevMarkers) => {
                ...
                if (existingMarkerIndex !== -1) {
                  ...
                  // Civic engagement was not previously visible, now it is (due to update)
                  markerAdded = true;
                  return [...prevMarkers, updatedMarker];
                }
              });
              ...
              console.error(
                "[useMapWebsocket] Error processing UPDATE_CIVIC_ENGAGEMENT:",
                e,
              );
            }
            break;
          ...
          case MessageTypes.DELETE_CIVIC_ENGAGEMENT: {
            if (!data.id) {
              console.warn(
                "[useMapWebsocket] Missing id in DELETE_CIVIC_ENGAGEMENT",
              );
              return;
            }
            try {
              const deletedId = `civic-${data.id}`; // Add prefix to match marker ID format
              let deleted = false;
              setMarkers((prevMarkers) => {
                const newMarkers = prevMarkers.filter(
                  (marker) => marker.id !== deletedId,
              ...
              console.error(
                "[useMapWebsocket] Error processing DELETE_CIVIC_ENGAGEMENT:",
                e,
              );
            }
            break;
          ...
          case MessageTypes.EVENT_DISCOVERED: {
            ...
            try {
              config.eventBroker.emit(config.EventTypes.EVENT_DISCOVERED, {
                timestamp: Date.now(),
                source: "useMapWebSocket",
          ...
          case MessageTypes.NOTIFICATION: {
            ...
            try {
              config.eventBroker.emit(config.EventTypes.NOTIFICATION, {
          ...
          case MessageTypes.XP_AWARDED: {
            ...
            const eventType =
              data.type === MessageTypes.LEVEL_UPDATE
                ? config.EventTypes.LEVEL_UPDATE
                : config.EventTypes.XP_AWARDED;
            try {
              config.eventBroker.emit(eventType, {
        ...
        setError(errorObj);
        config.eventBroker.emit(config.EventTypes.ERROR_OCCURRED, {
          timestamp: Date.now(),
          source: "useMapWebSocket",
          error: errorObj,
        });
    ...
    [config, emitMarkersUpdated], // No dependencies needed since convertEventToMarker has been moved to websocket service
    ...
    try {
      ...
      wsRef.current.onopen = () => {
        try {
          ...
          config.eventBroker.emit(config.EventTypes.WEBSOCKET_CONNECTED, {
            timestamp: Date.now(),
            source: "useMapWebSocket",
          });
          ...
          if (isAuthenticated && user?.id) {
            ...
            wsRef.current?.send(
              JSON.stringify({
                ...
                userId: user.id,
                clientType: config.clientType, // Use configurable client type
              }),
            );
          } else {
            console.warn(
      ...
      wsRef.current.onclose = (event) => {
        try {
          ...
          config.eventBroker.emit(config.EventTypes.WEBSOCKET_DISCONNECTED, {
            timestamp: Date.now(),
            source: "useMapWebSocket",
          });
  ...
  useEffect(() => {
    ...
    const unsubscribe = config.eventBroker.on(
      config.EventTypes.FORCE_VIEWPORT_UPDATE,
